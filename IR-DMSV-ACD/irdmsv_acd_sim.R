...
## HMC
options(mc.cores=4)
fit=model$sample(data=data_list,seed=1,chains = 4,iter_sampling = 3000,parallel_chains = 4,threads_per_chain = 12,iter_warmup=1000,thin=12,adapt_delta=0.85)
par_summary=fit$summary(variables = c("sigmasq[1]", "sigmasq[2]"
                                      ,"sigmasq[3]", "sigmasq[4]","phi[1]", "phi[2]","phi[3]","phi[4]","delta","omega","durpar[1]", "durpar[2]"
                                      ...
                                      ,"sigmasq_rho[5]", "sigmasq_rho[6]","meanar[1]", "meanar[2]","meanar[3]", "meanar[4]"
                                      ,"meanar[5]", "meanar[6]"))
...
filenamet<-sprintf("dmsv_g2_time1.rds")
filename0 <-sprintf("dmsv_g2_lpd1.rds")
filename <-sprintf("dmsv_g2_summ1.rds")
filename1 <-sprintf("dmsv_g2_full1.rds")
filename2 <- sprintf("dmsv_g2_waic1.rds")
...
alpha_fit <- fit$draws(c("durpar[1]", "durpar[2]"),format = "df")
...
filename3_1 <- sprintf("dmsv_g2_maetrain11.rds")
...
filename5_1 <- sprintf("dmsv_g2_maetest11.rds")
